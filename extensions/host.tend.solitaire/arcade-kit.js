...
function ensureKitStyles() {
  let style = document.getElementById(KIT_STYLE_ID);
  if (!style) { style = document.createElement('style'); style.id = KIT_STYLE_ID; document.head.appendChild(style); }
  style.textContent = `
...
export function createArcade(host, config = {}) {
  ...
  function selectMode(id) {
    const next = modes.find((m) => m.id === id && profile.rank >= (m.unlock || 1));
    if (!next) return false;
    profile.selectedMode = next.id; save(); if (hub) renderHub(); if (changeHandler) changeHandler({ mode: mode(), perk: perk(), profile }); return true;
  }
  function selectPerk(id) {
    const next = perks.find((p) => p.id === id && profile.rank >= (p.unlock || 1));
    if (!next) return false;
    profile.selectedPerk = next.id; save(); if (hub) renderHub(); if (changeHandler) changeHandler({ mode: mode(), perk: perk(), profile }); return true;
  }
  ...
  return Object.freeze({ ready, profile, mount, cleanup, record, reward, mode, perk, selectMode, selectPerk, isMode, hasPerk, showToast, openHub, closeHub, startPressure, resetPressure, stopPressure, onChange, save });
}
