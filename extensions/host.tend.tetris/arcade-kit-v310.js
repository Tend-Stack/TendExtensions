...
@media(min-width:768px){
  .tend-floating-window.tend-game-focus [data-extension-id]{display:flex!important;align-items:center!important;justify-content:center!important;overflow:hidden!important;background:radial-gradient(circle at 50% 35%,rgba(30,41,59,.46),rgba(2,6,23,.98) 68%)!important;padding:18px!important}
  .tend-floating-window.tend-game-focus .tend-arcade-root{width:var(--tend-arcade-native-w)!important;height:var(--tend-arcade-native-h)!important;max-width:calc(100dvw - 36px)!important;max-height:calc(100dvh - 36px)!important;flex:none!important;border-radius:18px!important;box-shadow:0 28px 90px rgba(0,0,0,.62)!important;overflow:hidden!important}
}
@media(max-width:767px){
  .tend-floating-window.tend-game-focus [data-extension-id]{padding:0!important}
  .tend-floating-window.tend-game-focus .tend-arcade-root{width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;border-radius:0!important}
}
`;
...
export function createArcade(host, config = {}) {
  ...
  function mount(nextRoot, nextContainer) {
    ...
    root.style.setProperty('--tend-arcade-native-w',Math.max(320,Math.round(nativeRect.width))+'px');
    root.style.setProperty('--tend-arcade-native-h',Math.max(420,Math.round(nativeRect.height))+'px');
  ...
  function renderHub() {
    ...
    const best = Math.max(0, Number.parseInt(root?.dataset?.tetrisBest || '0', 10) || 0);
    const bestStat = document.createElement('div');
    bestStat.className = 'tend-arcade-stat';
    bestStat.innerHTML = `<small>Best score</small><b style="color:#c084fc">${best.toLocaleString()}</b>`;
    hub.querySelector('.tend-arcade-stats')?.appendChild(bestStat);
    const selected = mode();
    const launch = document.createElement('div');
    launch.className = 'tend-arcade-launch';
    launch.innerHTML = `<div><small>SELECTED RUN</small><b>${selected.icon || '▦'} ${selected.name}</b><span>${selected.desc || ''} · Keyboard, touch, and full-screen ready</span></div><button class="tend-arcade-play" data-play type="button">PLAY ${selected.name.toUpperCase()}</button>`;
    hub.querySelector('.tend-arcade-stats')?.insertAdjacentElement('afterend', launch);
    const sections = hub.querySelectorAll('.tend-arcade-section');
    sections[0]?.classList.add('tend-arcade-modes');
    sections[1]?.classList.add('tend-arcade-perks');
    sections[2]?.classList.add('tend-arcade-daily');
    hub.querySelector('.tend-arcade-close').addEventListener('click', closeHub);
    hub.querySelector('[data-play]')?.addEventListener('click', () => {
      root?.dispatchEvent(new CustomEvent('tend-tetris-play', { detail: { mode: profile.selectedMode } }));
    });
